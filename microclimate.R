...
# It runs the model for the middle day of each month, and the input data is for Madison WI, USA
  ...
  setwd("source/") # set the working directory where the fortran source code is
...
############## location and climatic data  ###################################
  longlat<-c(-89.40123,43.07305) # type a long/lat here in decimal degrees, used if option 0 is chosen above
...
############# microclimate model parameters ################################
  ...
  HEMIS <- ifelse(longlat[2]<0,2.,1.) 
  ALAT <- abs(trunc(longlat[2]))
  AMINUT <- (abs(longlat[2])-ALAT)*60
  ALONG <- abs(trunc(longlat[1]))
  ALMINT <- (abs(longlat[1])-ALONG)*60
  azmuth<-aspect
  ...
  MAXSHADES <- rep(0,(julnum))+maxshade # daily max shade (%)
  MINSHADES <- rep(0,(julnum))+minshade # daily min shade (%)
  ...
  ALTT<-0 # altitude (m)
  TMAXX<-c(-3.2,0.1,6.8,14.6,21.3,26.4,29,27.7,23.3,16.6,7.8,-0.4)
  TMINN<-c(-14.3,-12.1,-5.1,1.2,6.9,12.3,15.2,13.6,8.9,3,-3.2,-10.6)
  RAINFALL<-c(28,28.2,54.6,79.7,81.3,100.1,101.3,102.5,89.7,62.4,54.9,41.2)
  CCMAXX<-c(50.3,47,48.2,47.5,40.9,35.7,34.1,36.6,42.6,48.4,61.1,60.1)
  CCMINN<-c(50.3,47,48.2,47.5,40.9,35.7,34.1,36.6,42.6,48.4,61.1,60.1)
  WNMAXX<-c(4.9,4.8,5.2,5.3,4.6,4.3,3.8,3.7,4,4.6,4.9,4.8)
  WNMINN<-c(4.9,4.8,5.2,5.3,4.6,4.3,3.8,3.7,4,4.6,4.9,4.8)
  RHMAXX<-c(100,100,100,100,100,100,100,100,100,100,100,100)
  RHMINN<-c(50.2,48.4,48.7,40.8,40,42.1,45.5,47.3,47.6,45,51.3,52.8)
  tannul<-mean(c(TMAXX,TMINN))
  tannulrun<-rep(tannul,julnum)
  SoilMoist<-c(0.424723267,0.416548157,0.416956055,0.430710144,0.439740356,0.438733734,0.428517365,0.417241486,0.414200195,0.415388885,0.422986786,0.42850769)
  ...
  moists2<-matrix(nrow=10, ncol = julnum, data=0)
  moists2[1,]<-SoilMoist
  moists2[2,]<-moists2[1,]
  moists2[3,]<-moists2[1,]
  moists2[4,]<-moists2[1,]
  ...
  soilprops[1,1]<-BulkDensity # soil bulk density
  ...
  soilprops[1,6]<-Density # mineral density
  soilprops[2,6]<-Density
  soilinit<-rep(tannul,length(DEP))
  ...
  metout<-as.data.frame(microut$metout[1:(julnum*24),]) # above ground microclimatic conditions, min shade
  shadmet<-as.data.frame(microut$shadmet[1:(julnum*24),]) # above ground microclimatic conditions, max shade
  soil<-as.data.frame(microut$soil[1:(julnum*24),]) # soil temperatures, minimum shade
  shadsoil<-as.data.frame(microut$shadsoil[1:(julnum*24),]) # soil temperatures, maximum shade
