...
setwd("sourcehr/") # set the working directory where the fortran source code is, assuming you are starting out in the /microclimate directory
cmnd<- "rcmd SHLIB micr2014hr.f BLKDATA.f dchxy.f dexpi.f DRYAIR.f DSUB.f error.f EVALXZ.f EVAP.f FUN.f gamma.f iomet1.f iomet2.f iosolr.f JREAD.f Micro.f MicroSegmt.f Osub.f Pttabl.f Rdctrl.f Rdtabl.f RelHumLocal.f Sfode.f sinec.f soilprops.f solrad.f Soylnods.f Tab.f VAPPRS.f vsine.f WETAIR.f ZBRAC.f ZBRENT.f"
system(cmnd) # run the compilation
setwd("..") # return to base directory

######################### times and location info #######################################################
longlat<-c(145.383, -37.7) # type a long/lat here in decimal degrees
julnum<-9 # number of time intervals to generate predictions for over a year (must be 12 <= x <=365)
julday<-seq(182,190) # middle day of each month
...
IRhr<-forecast$lw
SOLRhr<-forecast$sw
RAINhr<-forecast$prec

TMAXX<-aggregate(TAIRhr,by=list(forecast$day),max)[,2] # maximum air temperatures (deg C)
TMINN<-aggregate(TAIRhr,by=list(forecast$day),min)[,2] # minimum air temperatures (deg C)
RAINFALL<-aggregate(RAINhr,by=list(forecast$day),sum)[,2] # monthly mean rainfall (mm)
CCMAXX<-aggregate(CLDhr,by=list(forecast$day),max)[,2] # max cloud cover (%)
CCMINN<-aggregate(CLDhr,by=list(forecast$day),min)[,2] # min cloud cover (%)
RHMAXX<-aggregate(RHhr,by=list(forecast$day),max)[,2] # max relative humidity (%)
RHMINN<-aggregate(RHhr,by=list(forecast$day),min)[,2] # min relative humidity (%)
WNMAXX<-aggregate(WNhr,by=list(forecast$day),max)[,2] # max wind speed (m/s)
WNMINN<-aggregate(WNhr,by=list(forecast$day),min)[,2] # min wind speed (m/s
...
SoilMoist<-rep(0,length(TMAXX)) # soil moisture (decimal %)
...
write.table(TAIRhr, file = "TAIRhr.csv", sep = ",", col.names = NA, qmethod = "double")
write.table(RHhr, file = "RHhr.csv", sep = ",", col.names = NA, qmethod = "double")
write.table(WNhr, file = "WNhr.csv", sep = ",", col.names = NA, qmethod = "double")
write.table(CLDhr, file = "CLDhr.csv", sep = ",", col.names = NA, qmethod = "double")
write.table(IRhr, file = "IRhr.csv", sep = ",", col.names = NA, qmethod = "double")
write.table(SOLRhr, file = "SOLRhr.csv", sep = ",", col.names = NA, qmethod = "double")
write.table(RAINhr, file = "RAINhr.csv", sep = ",", col.names = NA, qmethod = "double")
setwd('..')

source('microrun_hrin.R') # Fortran wrapper for the microclimate model  
