...
C     Copyright Warren P. Porter 1998  All rights reserved.
C    Version 25 June 1998 incorporates skylight before sunrise and after sunset.  Spectral
...
C     REPINT(N)    RECIPROCAL OF EPINT 
      ...
      real altfct,altrad,alon,ALONC,amr,AMULT,AZMUTH
      real ar2,AZSLP,AZSUN,b,bothalf
      real c,cazsun,cdrnt,CMH2O,CCMAXX,CCMINN
      real cdrrnt,cgrnt,csrnt,cz,czsl
      real d,d0,DAY,dazsun,ddec,dec,delt,drint,drlam,drrint,drrlam,dzsl
      real ec,eom,epint,epl,eplam,er,erlam,elog
      real fd,fdav,fdq,fdqdav,FRACT,FRACTR,FRACTS,grint
      real global,gpl,grlam,h,ha,had,hemis,hh,HHINT,HRINT,HSINT,hzen
      real julday,MAXSHADES,MAXSHD,MINSHADES
      real ocgrnt,ODNEW,ODRNT,ODSTOR,OSTORE,OSNEW
      real oz,OZNEW,ozone,ozsl,ozstor,ozz
      real part1,part2,PCTWET,pi,PRESS,PTWET,PUNSH
      REAL REFL,refr,repint,RHMAXX,RHMINN
      real s,SABNEW,se,SHAYD,sinde,slam
      ...
      real vmax,vmin,w,WNMAXX,WNMINN,z,zsl,ZSLNEW,zslsto,zz
     ...
     & ,I94,I95,I96 
      ...
      DIMENSION TMINN(7300),TMAXX(7300),DAY(7300)
      DIMENSION OSNEW(25),OSTORE(25),OZNEW(25),TNEW(25),ZSLNEW(25)
      DIMENSION CCMINN(7300),CCMAXX(7300),RHMINN(7300),RHMAXX(7300)
      DIMENSION WNMINN(7300),WNMAXX(7300),TIMINS(4),TIMAXS(4)
      dimension julday(7300),MAXSHADES(7300),MINSHADES(7300)
      DIMENSION SNOW(7300),REFLS(7300),PCTWET(7300),sles(7300)
      DIMENSION hori(24),azi(24),tai(111),RAIN(7300),SNOWHR(25*7300)
...
c  188 FORMAT(1A1)
c  189 FORMAT(20A4)  
c  191 FORMAT(1X,20A4)   
c  195 FORMAT(1X,1A1)
c  199 FORMAT(1H0,'SCATTERED COMPONENT NOT COMPUTED')
  351 FORMAT(1H0,'TERRESTRIAL HORIZONTAL PLANE INTEGRATED FLUXES(MW-CM-2
     * AND CAL-CM-2-MIN-1)')
  352 FORMAT(1H ,'TOTAL DIRECT COMPONENT RADIATION = ',E10.3,' MW-CM-2 =
     * ',E10.3,' CAL-CM-2-MIN-1')   
  353 FORMAT(1H ,'DIFFUSE RAYLEIGH COMPONENT =       ',E10.3,' MW-CM-2  
     1= ',E10.3,' CAL-CM-2-MIN-1')  
  354 FORMAT(1H ,'GLOBAL RADIATION COMPONENT =       ',E10.3,' MW-CM-2  
     1= ',E10.3,' CAL-CM-2-MIN-1')  
  355 FORMAT(1H0,'WAVELENGTH',3X,'SPECTRAL(MW-CM-2-NMETER-1)',15X,'  INT
     XEGRAL(MW-CM-2)')  
  356 FORMAT(1H ,'(NANOMTRS)',3X,'DIRECT    DIFFUSE    GLOBAL',10X,'DIRE
     XCT    DIFFUSE    GLOBAL')     
  357 FORMAT(1H ,3X,I4,2X,E10.3,1X,E10.3,E10.3,6X,E10.3,1X,E10.3,E10.3) 
  358 FORMAT(1H0,'TOTAL DIRECT RAYLEIGH COMPONENT  = ',E10.3,' MW-CM-2  
     1= ',E10.3,' CAL-CM-2-MIN-1')  
  359 FORMAT(1H ,'WAVELENGTH',5X,'SPECTRAL(SEC-1-NM-1)',5X,'  INTEGRATED
     X(SEC-1)',5X,'INTEGRATED-1(MIN)')  
  360 FORMAT(1H ,3X,I4,8X,E10.3,18X,E10.3,12X,E10.3)
  361 FORMAT(1H0,30X,'ERYTHEMAL PRODUCT')   
  362 FORMAT(1H0,'MINIMAL ERYTHEMA PRODUCING TIME IS ',F8.2,' MINUTES') 
  363 FORMAT(1H ,'ERYTHEMA PRODUCT CURVE MAXIMUM AT ',I4,' NANOMETERS') 
  364 FORMAT(1H ,I4,10X,E10.3,10X,E10.3)
  365 FORMAT(1H ,'TWO PRODUCT POINTS ARE EQUAL, SO CHOSE FROM FOLLOWING 
     X')    
  366 FORMAT(1H0,'WVLTH',11X,'ERYTH. PROD.',10X,'GLOBAL')   
  475 FORMAT(1H0,'LONG NIGHT-NO RADIATION')     
  476 FORMAT(1H0,'TIME OF DAY AT OR BEFORE SUNRISE')
  478 FORMAT(1H0,'TIME OF DAY AT OR AFTER SUNSET')  
c  500 FORMAT(I4,F5.1,F4.2,F4.3,F5.2,F6.3)   
c  501 FORMAT (F3.2)     
c  502 FORMAT(12F5.3)    
c  503 FORMAT(F5.0)  
c  504 FORMAT(F5.2)  
c  505 FORMAT(E10.3)     
c  509 FORMAT(1H ,10X,'LON. CORR=',F6.2,' HOURS')
  510 FORMAT(1H0,'SOLAR RADIATION FOR A TERRESTRIAL HORIZONTAL SURFACE O
     XN ',I2,'/',I2)    
  511 FORMAT(1H ,'AT ',F5.1,' HOURS (LOCAL STANDARD TIME) FOR LATITUDE O
     XF ',F5.1,' DEGREES AND A')    
C 512 FORMAT(1H ,10X,'ALBEDO =',4X,F4.2)
C 513 FORMAT(1H ,10X,'PRESS.=  ',F6.1,'  MB',40X,'NOSCAT=',I3)   
C 514 FORMAT(1H ,10X,'MET.RANGE=',F5.1,'  KM',40X,'NMAX=',I3)
C 515 FORMAT(1H ,10X,'CMH2O=',4X,F6.2,' CM')    
C 516 FORMAT(1H ,10X,'OZONE=',4X,F6.2,' CM',40X,'IPINT=',I3) 
C 517 FORMAT(1H ,10X,'AIR MASS=',F7.2)  
C 518 FORMAT(///1H0,'SOLAR RADIATION FOR A TERRESTRIAL HORIZONTAL SURFAC
C    1E ON ',I2,'/',I2) 
C 520 FORMAT(1H ,10X,'DECLIN.=  ',F5.1,'  DEGREES') 
C 521 FORMAT(1H ,10X,'SUNSET AT',F6.1,2X,'HOURS AFTER SOLAR NOON',20X,  
C    1'IUV=',I3) 
c  522 FORMAT(1H ,10X,'ZEN. ANG.=',F5.1,'  DEGREES',35X,'IEP=',I3)
C 523 FORMAT(2E10.3,F4.3)   
C 524 FORMAT(1H ,10X,'SOL. NOON=',F6.2,' HOURS (STD. TIME)')
C 525 FORMAT(F5.3,2X,F5.3,2X,F5.3,2X,F5.3)  
C 526 FORMAT(1H ,10X,'ALTITUDE=  ',I2,4X,'KM')  
C 527 FORMAT(5E10.3)    
C 528 FORMAT(11F4.3)    
C 529 FORMAT(1H ,E10.3,5X,E10.3,5X,F4.3,5X,E10.3)   
C 530 FORMAT(6F7.5)     
C 531 FORMAT(7E10.3)    
C 532 FORMAT(6E10.3)    
C 533 FORMAT(5E10.3)    
...
C 537 FORMAT(11F4.2)    
  540 FORMAT (2E15.5)   
  541 FORMAT(1X,'TAB',I3,1X,'SOL')  
  542 FORMAT(1X,'TAB',I3,1X,'ZEN')  
  543 FORMAT(1X,'TTL 1')
  544 FORMAT(1X,'MONTH/DAY = ',1X,I2,'/',I2)  
  545 FORMAT(1X,'TAB',I3,1X,'ZSL')  
C 546 FORMAT(1X,'DAY',I2,' OF MONTH/DAY ',I2,'/',I2)
C 547 FORMAT(1X,'TAB',I3,1X,'DIR')  
C 605 FORMAT(1X,I4,F5.1,F4.2,F4.3,F5.2,F6.3)   
C 606 FORMAT(1X,12F5.3)    
C 607 FORMAT(1X,8F6.1)
c  623 FORMAT(11X,'HOUR ANG. = ',1F6.0,' DEGREES')
  624 FORMAT(11X,'SLOPE ELEVATION ANGLE = ',3X,1F5.1,' DEGREES')
  625 FORMAT(11X,'SLOPE AZIMUTH ANGLE = ',3X,1F7.1,' DEGREES')
  626 FORMAT(11X,'SLOPE SOLAR ZENITH ANGLE = ',1F5.1,' DEGREES')
  627 FORMAT(11X,'SLOPE SOLAR AZIMUTH ANGLE (North reference) = ',1F7.1)
  629 FORMAT(11X,'SLOPING SURFACE IN THE NORTHERN HEMISPHERE')
  630 FORMAT(11X,'SLOPING SURFACE IN THE SOUTHERN HEMISPHERE')
c  631 FORMAT('+',1X,'SOLRAD: ON ',I2,'/',I2,' THE TIME IS ',
c     & 1F4.0,' HOURS')
  632 FORMAT(11X,'SOLAR AZIMUTH ANGLE (North reference) = ',1F7.1)
c  633 FORMAT(11X,'SOLAR AZIMUTH ANGLE (South reference) = ',1F7.1)
c  990 FORMAT(1X,2F4.0,1E12.4)
...
c      READ(I8,500)(ILAM(I),SLAM(I),TR(I),TA(I),TO(I),TW(I),I=1,111)  

c      READ(I8,502)((OZ(L,M),M=1,12),L=1,19)  
...
c      READ(I8,525)((ALTFCT(L,M),M=1,4),L=1,21)  
c      READ(I8,532)((FD(I,J),I=1,6),J=1,19)
c      READ(I8,527)((FD(I,J),I=7,11),J=1,19) 
c      READ(I8,532)((FDQ(I,J),I=1,6),J=1,19)  
c      READ(I8,527)((FDQ(I,J),I=7,11),J=1,19) 
c      READ(I8,528)(S(I),I=1,11)  
      

      
c      READ(I8,535)(IELAM(J),J=1,11)
c      IELAM(12)=305  
c      DATA IELAM/300,302,304,306,308,310,312,314,316,318,320,305/
...
c      READ(I8,531)(ER(J),J=1,7) 


      
c      READ(I8,170)(EOM(J),J=1,13) 
...
c      WRITE(7,529) FD(4,8),FDQ(4,8),S(9),ER(6)  
...
C    SCREEN MESSAGE THAT SOLRAD IS WORKING
c    WRITE(0,*)'Solar radiation, air temp. calculations starting.'
c    WRITE(0,*)' '

C     **** USRDAT ****  
C     READ LOCALITY LABEL, INCLUDE LATITUDE AND LONGITUDE   
C     READ(I8,188,END=999)(LABL(I),I=1,60)

C     **** USRDAT ****  
C     READ(I8,*) NMAX,IPINT,NOSCAT,IUV,IEP 
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then  
        WRITE(I9,538) NMAX,IPINT,NOSCAT,IUV,IEP  
      Endif 
 538  FORMAT (1X,5I5)   
...
C     **** USRDAT ****  
C     READ(I8,*) IALT,ALAT,PRESS,AMR,CMH2O,REFL,ALONC, PUNSH
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then 
        WRITE(I9,*)IALT,ALAT,PRESS,AMR,CMH2O,REFL,ALONC,PUNSH 
      Endif
c      WRITE(i2,*)'IALT,ALAT,PRESS,AMR,CMH2O,REFL,ALONC,PUNSH' 
c      WRITE(i2,333)IALT,ALAT,PRESS,AMR,CMH2O,REFL,ALONC,PUNSH 
c      WRITE(i2,*)' '
c333   FORMAT(1X,1I4,7F10.3)
...
C     READ (5,*)(julday(IDAY),IDAY=1,IDA)
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
        WRITE(I9,168)(julday(IDAY),IDAY=IDAYST,IDA) 
      Endif  
  168 FORMAT(13F5.0)    
...
C     40 = long day,  41 = long night
   ...
   42 Continue
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
c        WRITE (I9,518)M,IDOM
      Endif  
   43 Continue
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
        WRITE(I9,511) TD,ALAT 
C      'LONG NIGHT-NO RADIATION')     
        WRITE(I9,475)
      Endif     
...
C    Write Month, Day of the month
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then      
        WRITE (I9,510)M,IDOM 
      Endif 
      GO TO 34 
...
C      LLAT =1 IS 90 DEGREES SOUTH AND LLAT =19 IS 90 DEGREES NORTH     
  ...
  150 OZONE=OZ(LLAT,M)  
      ...
      HAD = 15.*(TD - TSN)
C     If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
C       WRITE(I9,510) M,IDOM  
C       WRITE(I9,511)TD,ALAT  
C       WRITE(I9,517)AIRMS
C       WRITE(I9,512)REFL 
C       WRITE(I9,526)IALT 
C       WRITE(I9,515)CMH2O
C       WRITE(I9,520) DDEC
C       WRITE(I9,509) ALONC   
C       WRITE(I9,514)AMR,NMAX 
C       WRITE(I9,516) OZONE,IPINT     
C       WRITE(I9,513)PRESS,NOSCAT     
C       WRITE(I9,521)HH,IUV   
C       WRITE(I9,522)ZZ,IEP   
C       WRITE(I9,623)HAD
C     Endif

C    Solar azimuth angle output
      If ((slope .eq. 0.) .and. (amult .gt. 0.0)) then
C      Northern hemisphere
        If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
          WRITE(I9,632)DAZSUN
        Endif
       else
        If ((slope .eq. 0.) .and. (amult .lt. 0.0)) then
C        Southern hemisphere
          If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
            WRITE(I9,632)DAZSUN
          Endif
      ...
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then        
C       WRITE ONLY IF SLOPING LAND
        IF (SLOPE .GT. 0.0000) THEN
          IF (AMULT .EQ. 1.) THEN
              WRITE(I9,629)
           ELSE
            WRITE(I9,630)
...
c    polynomials to the ALTFCT data in SOLAR.DAT    
      ALT=ALTT/1000+1 
...
c      TLAM1=(PRESS/1013.)*TR(N)*ALTFCT(MALT,1)  
c      TLAM2=(25./AMR)*TA(N)*ALTFCT(MALT,2)  
c      TLAM3=(OZONE/0.34)*TO(N)*ALTFCT(MALT,3)   
c      TLAM4=TW(N)*SQRT(AIRMS*CMH2O *ALTFCT(MALT,4)) 
      TLAM=((real(TLAM1)+TLAM2+TLAM3)*AIRMS)+TLAM4    
...
C    SO THE INTEGRATOR DOESN'T GET CONFUSED AT VERY LOW SUN ANGLES
      IF (DRLAM(N) .LT. 0.1E-24) THEN
        DRLAM(N) = 0.1E-24
      ENDIF
      DRRLAM(N)=(SLAM(N)*AR2*CZ)*EXP(-1.*real(TLAM1)*AIRMS)/1000. 
...
C     CONSUMING AND COSTLY GAMMA SUBROUTINE.    
  ...
  403 IF(TLAM1 .LT. 0.03) GO TO 400 
      ...
      SRLAM(N)=(((real(GAML(INTCZ))+real(GAMR(INTCZ)))/(2.*(1.-REFL*real
     X(SBAR))))-EXP(-real(TLAM1)*AIRMS))*CZ*SLAM(N)*AR2/1000. 
      GO TO 401 
  400 SRLAM(N)=0.0  
...
C     REFERENCE--- PHYSICS IN MEDICINE AND BIOLOGY, VOLUME 15, PAGE 723(1970)   
      ...
      A(1)=(ALOG(GRLAM(4))-ALOG(GRLAM(3)))/(ILAM(4)-ILAM(3))
      A(2)=(ALOG(GRLAM(5))-ALOG(GRLAM(4)))/(ILAM(5)-ILAM(4))
      A(3)=(ALOG(GRLAM(6))-ALOG(GRLAM(5)))/(ILAM(6)-ILAM(5))
      A(4)=(ALOG(GRLAM(7))-ALOG(GRLAM(6)))/(ILAM(7)-ILAM(6))
      GPL(1)=GRLAM(3)   
      GPL(2)=EXP(ALOG(GRLAM(3))+(A(1)*(302.-300.))) 
      GPL(3)=EXP(ALOG(GRLAM(3))+(A(1)*(304.-300.))) 
      GPL(4)=EXP(ALOG(GRLAM(4))+(A(2)*(306.-305.))) 
      GPL(5)=EXP(ALOG(GRLAM(4))+(A(2)*(308.-305.))) 
      GPL(6)=GRLAM(5)   
      GPL(7)=EXP(ALOG(GRLAM(5))+(A(3)*(312.-310.))) 
      GPL(8)=EXP(ALOG(GRLAM(5))+(A(3)*(314.-310.))) 
      GPL(9)=EXP(ALOG(GRLAM(6))+(A(4)*(316.-315.))) 
      GPL(10)=EXP(ALOG(GRLAM(6))+(A(4)*(318.-315.)))
      GPL(11)=GRLAM(7)  
      DO 1510 MOM=1,11  
...
C     REPINT(N)    RECIPROCAL OF EPINT  
      ...
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then   
        WRITE(I9,355)     
        WRITE(I9,356)     
        WRITE(I9,357)(ILAM(K),DRLAM(K),SRLAM(K),GRLAM(K),DRINT(K),
     *  SRINT(K),GRINT(K),K=1,NMAX)
      Endif   
C     WRITING PLOT FILE
      IF (IT .LE. IEND) THEN
...
997     CONTINUE
      ...
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
        WRITE(I9,358)DRRINT(NMAX)  
      Endif   
      IF(IEP .EQ. 1) GO TO 1750
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then     
        WRITE(I9,361)     
        WRITE(I9,359)     
        WRITE(I9,360)(ILAM(K),EPLAM(K),EPINT(K),REPINT(K),K=1,7)
      Endif
        
 1750 Continue
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
        WRITE(I9,362)REPINT(7)
      Endif
...
c 1800 CONTINUE 
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then 
        WRITE(I9,366)     
        WRITE(I9,364)(IELAM(J),EPL(J),GPL(J),J=1,3 )  
        WRITE(I9,364)ILAM(4),EPL(12),GRLAM(4)     
        WRITE(I9,364)(IELAM(J),EPL(J),GPL(J),J=4,11)
      Endif  
      GO TO 200 
  350 Continue
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
        WRITE(I9,351)
      Endif
      If (Amult .eq. 1.0) then
        If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
          Write(I9,*)'NORTHERN HEMISPHERE  ***'
        Endif
       else
        If (Amult .eq. -1.0) then
          If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
            Write (I9,*)'SOUTHERN HEMISPHERE ***'
          Endif
        Endif
      Endif         
...
C    THEY ARE EASIEST TO GET JUST FROM THE COS (ZENITH ANGLE). ONLY WANT OUTPUT FOR SLOPE SUN ANGLE, SO CAN DO CORRECTION IN DSUB FOR MICROMET CALC'S. 
      ...
      CGRNT=GRINT(NMAX)*.01434 
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
        WRITE(I9,352) DRINT(NMAX),CDRNT   
        WRITE(I9,358)DRRINT(NMAX),CDRRNT 
      Endif 
      IF(NOSCAT .EQ. 0) GO TO 198
      If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then   
        WRITE(I9,353) SRINT(NMAX),CSRNT   
        WRITE(I9,354) GRINT(NMAX),CGRNT
      Endif   
      GO TO 1889
  198 Continue
C     If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then
...
C        WRITE(I4,543)     
C        WRITE(I4,544)M,IDOM  
c        WRITE (I4,541) NCT
c        WRITE (I4,540) (TSTOR(JJ),OSTORE(JJ),JJ=1,NCT)
          do 2015 jj=1,25
...
2015      continue
        If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then   
          ...
          WRITE(I9,544)M,IDOM  
          WRITE (I9,541) NCT
          WRITE(I9,540)(TSTOR(JJ),OSTORE(JJ),JJ=ISTART,NCT)
        Endif     
C       "PUNCHING" DIRECT SOLAR OUT FOR MICROMET INPUT
C        WRITE(I4,543)     
C        WRITE(I4,544)M,IDOM  
c        WRITE (I4,547) NCT
c        WRITE (I4,540) (TSTOR(JJ),ODNEW(JJ),JJ=1,NCT)   
c        WRITE(I9,543)     
c        WRITE(I9,544)M,IDOM  
c        WRITE (I9,541) NCT
c        WRITE(I9,540)(TSTOR(JJ), ODNEW(JJ),JJ=1,NCT)     

C       " PUNCHING" ZENITH ANGLES OUT   
c        WRITE (I4,542) NCT
c        WRITE (I4,540) (TSTOR(JJ),OZSTOR(JJ),JJ=1,NCT)
          do 2013 jj=1,25
...
2013      continue
        ...
        If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then   
          WRITE (I9,542) NCT
          WRITE(I9,540)(TSTOR(JJ),OZSTOR(JJ),JJ=1,NCT) 
        Endif    

        IF (SLOPE .GT. 0.00) THEN
C         " PUNCHING" SLOPE ZENITH ANGLES OUT   
c          WRITE (I4,545) NCT
c          WRITE (I4,540) (TSTOR(JJ),ZSLSTO(JJ),JJ=1,NCT)
          cnt=cnt-1
          do 2014 jj=1,25
...
2014      continue
          ...
          If ((solout .eq. 'Y').or.(solout .eq. 'Y'))then   
            WRITE (I9,545) NCT
            WRITE(I9,540)(TSTOR(JJ),ZSLSTO(JJ),JJ=1,NCT)
          Endif     
        ENDIF
       ELSE
      IF (IPINT .EQ. 0) THEN
...
730        CONTINUE
          ELSE
         ENDIF
c      WRITE(11,990)(julday(IDAY),TIME(IT),OSTORE(IT),IT=1,25)
       ELSE
C      WRITE PLOT DATA: JULIAN DAY, HOUR OF DAY, WAVELENGTH, SOLAR
        IF (NOSCAT .EQ. 0) THEN
C        DIRECT ONLY
c        WRITE(11,991)(julday(IDAY),TIME(IT),ILAM(IT),DRLAM(IT),
c     *      IT=1,25)
         ELSE
C        GLOBAL
c        WRITE(11,991)(julday(IDAY),TIME(IT),ILAM(IT),GRLAM(IT),
c     *      IT=1,25)
        ENDIF
      ENDIF
      ENDIF
...
C     ALL DONE, WRITE 'STP' FOR STOPPING MICROMET PROGRAM
c      WRITE(I4,*)'STP'
      RETURN
      END  
